    ...
    return await prisma_1.default.event.findMany({
        where: {
            ...
            startDate: filter.startDate || filter.endDate
                ? Object.assign(Object.assign({}, (filter.startDate && { gte: filter.startDate })), (filter.endDate && { lte: filter.endDate })) : undefined,
        },
...
async function getTotalEvents(query) {
    ...
    return await prisma_1.default.event.count({
        where: Object.assign({ name: filter.name ? { contains: filter.name } : undefined, categoryId: filter.categoryId, locationId: filter.locationId }, (filter.startDate || filter.endDate
            ? {
                startDate: Object.assign(Object.assign({}, (filter.startDate && { gte: filter.startDate })), (filter.endDate && { lte: filter.endDate })),
            }
            : {})),
    });
}
