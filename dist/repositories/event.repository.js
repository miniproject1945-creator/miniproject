...
export class EventRepository {
    ...
    static async getTotalEvents(query) {
        const filter = {
            price: query.price ? Number(query.price) : undefined,
            locationId: query.locationId ? Number(query.locationId) : undefined,
            categoryId: query.categoryId ? Number(query.categoryId) : undefined,
    ...
    static async getTotalEventsBySearch(query) {
        const filter = {
            name: query.name ? String(query.name) : undefined,
            locationId: query.locationId ? Number(query.locationId) : undefined,
            categoryId: query.categoryId ? Number(query.categoryId) : undefined,
    ...
    static async getEventByIdWithInclude(query) {
    ...
    static async getEventByIdWithTransaction(eventId, userId) {
    ...
    static async getEventIncludeTransactionWithPagination(id, query) {
    ...
    static async countEventTransactions(eventId) {
        return await prisma.event.findUnique({
            where: { id: eventId },
            select: { _count: { select: { transactions: true } } },
        });
    }
    static async getEventIncludeCategoryLocation(id) {
        return await prisma.event.findUnique({
            where: { id },
            include: { category: true, location: true },
        });
